...
test('use customElementRef to get an ref to the custome-element and compute its height', withPage, async (t, page) => {
  ...
  await page.evaluate(() => (window.CalculatedHeight = () => {
    ...
    return <SuperParagraph style={{ height: '99px', display: 'block' }} customElementRef={ref}>My calculated height: {height}</SuperParagraph>
  }))
  await page.evaluate(() => {
