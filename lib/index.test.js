...
test('use innerRef to get an ref to the custom-element and compute its height with useRef', withPage, async (t, page) => {
  ...
  await page.evaluate(() => (window.CalculatedHeight = () => {
    ...
    return <SuperParagraph style={{ height: '99px', display: 'block' }} innerRef={ref}>My calculated height: {height}</SuperParagraph>
  }))
  await page.evaluate(() => {
  ...
  await page.waitFor(100) // Wait until the React.useEffect has run and updated the height
  t.is(await page.evaluate(() => document.querySelector('#react-app').textContent), 'My calculated height: 99')
})

test('use innerRef to get an ref to the custom-element and compute its height using callback ref', withPage, async (t, page) => {
  await page.evaluate(() => (window.SuperParagraph = customElementToReact(class SuperParagraph extends HTMLElement {
  })))
  await page.evaluate(() => (window.CalculatedHeight = class CalculatedHeight extends React.Component {
    ...
    render () {
      ...
      return <SuperParagraph style={{ height: '99px', display: 'block' }} innerRef={this.setElementRef}>My calculated
        height: {height}</SuperParagraph>
    }
  }))
  await page.evaluate(() => {
    let divElement = document.createElement('div')
...
test('use innerRef to get an ref to the custom-element and compute its height using React.createRef', withPage, async (t, page) => {
  await page.evaluate(() => (window.SuperParagraph = customElementToReact(class SuperParagraph extends HTMLElement {
  })))
  await page.evaluate(() => (window.CalculatedHeight = class CalculatedHeight extends React.Component {
    constructor (props) {
    ...
    render () {
      ...
      return <SuperParagraph style={{ height: '99px', display: 'block' }} innerRef={this.elementRef}>My calculated
        height: {height}</SuperParagraph>
    }
  }))
  await page.evaluate(() => {
