...
import customElementToReact from './index.js'
...
async function withPage (t, run) {
  ...
  page.on('console', msg => msg && msg._text && console.log(msg._text))
...
test('converts from simple class', withPage, async (t, page) => {
  ...
  await page.evaluate(() => (window.comp = ReactDOM.render(React.createElement(Test), document.createElement('div'))))
...
test('use forwardRef to get an ref to the custom-element and compute its height with useRef', withPage, async (t, page) => {
  ...
  await page.evaluate(() => (window.CalculatedHeight = () => {
    ...
    return (
      React.createElement(
        SuperParagraph,
        {
          style: { height: '99px', display: 'block' },
          forwardRef: ref
        },
  ...
  await page.evaluate(() => (window.comp = ReactDOM.render(React.createElement(CalculatedHeight), document.querySelector('#react-app'))))
...
test('converts from class with props and events', withPage, async (t, page) => {
  ...
  await page.evaluate(() => (window.comp = ReactDOM.render(React.createElement(Test, { prop1: 1234, prop3: true, onEvent1: () => {} }), document.createElement('div'))))
...
test('sets tag name from capitalized', withPage, async (t, page) => {
  ...
  await page.evaluate(() => (window.comp = ReactDOM.render(React.createElement(TestTest), document.createElement('div'))))
...
test('stringifies correctly for server side rendering', (t) => {
  ...
  const ReactElementTest = React.createElement(Test, { prop1: '', prop2: '', prop3: true, disabled: true, onEvent1: () => {} })
  const stringified = ReactDOMServer.renderToString(ReactElementTest)
  t.is(stringified, '<test-1234 prop3="" disabled=""></test-1234>')
})
...
test('hydrates custom element before definition', withPage, async (t, page) => {
  ...
  const stringified = ReactDOMServer.renderToString(React.createElement(Test))
  t.is(stringified, '<test-react></test-react>')
  ...
  await page.evaluate(() => (window.test = React.createElement(Test)))
