...
test('converts from simple class', withPage, async (t, page) => {
  await page.evaluate(() => (window.Test = window.customElementToReact(class Test extends window.HTMLElement {})))
  await page.evaluate(() => (window.comp = window.ReactDOM.render(<Test />, document.createElement('div'))))
  t.true(await page.evaluate(() => Test instanceof Function))
  t.true(await page.evaluate(() => comp instanceof window.React.Component))
  t.is(await page.evaluate(() => window.ReactDOM.findDOMNode(comp).nodeName), 'TEST-REACT')
})
