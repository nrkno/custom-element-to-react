...
export default function customElementToReact (element, options = {}) {
  let name = element.name || String(element).match(/function ([^(]+)/)[1] // String match for IE11
  name = name.replace(/[A-Z]/g, m => `-${m}`).replace(/^-/, '').replace(/\W+/, '-').toLowerCase() // Name -> name, NameName -> name-name
  const props = options.props || []
  const events = options.customEvents || []
  const tag = `${name}-${options.suffix || 'react'}`
