...
export default function customElementToReact (elementClass, options = {}) {
  ...
  const tag = `${dashCase}-${options.suffix || 'react'}`.replace(/\W+/, '-').toLowerCase()
  ...
  return class extends React.Component {
    ...
    componentDidMount () {
      props.forEach((key) => this.props.hasOwnProperty(key) && (this.el[key] = this.props[key]))
    ...
    render () {
      ...
      return React.createElement(tag, Object.keys(this.props).reduce((props, key) => {
        ...
        else if (this.props[key] !== false && !this[key]) props[key] = this.props[key] // Pass only truthy, non-function props
        return props
      }, { ref: this.ref }))
    }
  }
